  ...
  async integrate(
    ...
    for (const artifact of artifacts) {
      const merged = await integration.run(
        [
          "-c", "user.name=agent-code Coordinator",
          "-c", "user.email=coordinator@agent-code.invalid",
...
export class SubagentCoordinator {
  ...
  async run(
    ...
    const batchRoot = await mkdtemp(join(this.#temporaryDirectory, "agent-code-subagents-"));
    ...
    try {
      ...
      for (const [index, item] of validated.entries()) {
        ...
        const branch = `agent-code/subagent-${safeRef(this.#createId())}-${index + 1}`;
  ...
  async #runTask(
    ...
    for (const args of [
      ["add", "--all"],
      [
        "-c", "user.name=agent-code Subagent",
        "-c", "user.email=subagent@agent-code.invalid",
        "commit", "-m", `agent-code subagent: ${prepared.task.id}`,
      ],
    ] as const) {
