...
export const PATCH_FORMAT_GUIDE = [
  ...
  "Use '*** Add File: path' with every content line prefixed by '+' to add a file; parent directories are created automatically.",
...
export async function applyWorkspacePatch(
  ...
  // Auto-create parent directories for Add File so models don't need run_shell mkdir first.
  if (patch.operation === "add") {
    await mkdir(dirname(resolve(guard.root, path)), { recursive: true });
  }
...
async function atomicWrite(
  ...
  const target = await guard.resolveForWrite(path);
  const temporary = join(dirname(target.lexicalPath), `.${basename(path)}.dugsyn-${randomUUID()}.tmp`);
  let created = false;
  try {
