    ...
    async handler(input, context) {
      ...
      const discoveryComplete = await walkDirectory(
        ...
        async (entry) => {
          ...
          else if (entry.kind === "directory") dirCount++;
      ...
      const page = paginateLines(
        ...
        `workspace: ${resolved.relativePath || "."}\nentries: ${lines.length} (${fileCount} files, ${dirCount} dirs)${discoveryComplete ? "" : "+ (limit reached)"}`,
      );
      return {
...
function createReadFileTool(guard: WorkspacePathGuard, maxFileBytes: number): Tool {
  return {
    definition: {
      name: "read_file",
      description: "Read a UTF-8 workspace file. Omit maxLines to read as much as fits; the output is paginated with a cursor when it exceeds the limit. Returns SHA-256 for apply_patch.",
      inputSchema: objectSchema({
    ...
    async handler(input, context) {
      ...
      const maxLines = numberValue(input, "maxLines", lines.length);
      const scope = `read:${resolved.relativePath}:${hash}`;
      const offset = decodeCursor(optionalString(input, "cursor"), scope);
      ...
      const page = paginateLines(slice, 0, 64 * 1024, header);
...
function createStatFileTool(guard: WorkspacePathGuard): Tool {
  return {
    definition: {
      name: "stat_file",
      description: "Check whether a workspace path exists, its type, and file size without reading contents. Call to get file size before read_file so you can decide whether to paginate with maxLines. Skip read_file if the file is not found.",
      inputSchema: objectSchema({
        path: { type: "string", minLength: 1, maxLength: 4_096 },
      }, ["path"]),
    },
    ...
    async handler(input, context) {
      ...
      try {
        ...
        return {
          ...
          data: { exists: true, kind, path: resolved.relativePath || ".", size: metadata.size },
        };
      } catch {
        return {
