...
export interface ToolRegistryOptions {
  ...
  /** When true, read_file is blocked unless stat_file was called first on the same path. */
  readonly enforceStatBeforeRead?: boolean | undefined;
}
...
export class ToolRegistry {
  ...
  readonly #permissions: PermissionEngine | undefined;
  readonly #hooks: HookRunner | undefined;
  readonly #enforceStatBeforeRead: boolean;
  ...
  constructor(tools: readonly Tool[], options: ToolRegistryOptions = {}) {
    ...
    this.#enforceStatBeforeRead = options.enforceStatBeforeRead ?? false;
  ...
  async #execute(
    ...
    statChecked: Set<string> | undefined,
    ...
    // Enforce: read_file requires a prior stat_file on the same path
    if (call.name === "read_file" && statChecked !== undefined) {
      const readPath = typeof call.input.path === "string" ? call.input.path.trim() : "";
    ...
    try {
      ...
      // Record stat_file + list_files paths so read_file can validate them
      if (statChecked !== undefined) {
        if (call.name === "stat_file") {
          const statPath = typeof call.input.path === "string" ? call.input.path.trim() : "";
          if (statPath.length > 0) statChecked.add(statPath);
        } else if (call.name === "list_files" && typeof output.content === "string") {
          // Extract file paths from list_files output (skip dirs ending with / and symlinks)
          for (const line of output.content.split("\n")) {
            const trimmed = line.trim();
            if (trimmed.length === 0 || trimmed.startsWith("workspace:") || trimmed.startsWith("entries:")) continue;
