...
export interface LoadedConfiguration {
  ...
  /** Explicit DUGSYN_MODEL override, independent of the selected provider. */
...
function resolvePaths(
  ...
  const managed = configured?.managed ?? nonEmpty(environment.DUGSYN_MANAGED_CONFIG);
  const user = configured?.user ?? nonEmpty(environment.DUGSYN_USER_CONFIG) ??
    join(homedir(), ".dugsyn", "config.json");
  return {
    ...(managed === undefined ? {} : { managed: resolveFrom(cwd, managed) }),
    user: resolveFrom(cwd, user),
    project: resolve(configured?.project ?? join(workspaceRoot, ".dugsyn", "config.json")),
    local: resolve(configured?.local ?? join(workspaceRoot, ".dugsyn", "config.local.json")),
  };
}
...
function parseEnvironment(environment: NodeJS.ProcessEnv, cwd: string): Configuration & { model?: string } {
  const provider = nonEmpty(environment.DUGSYN_PROVIDER);
  ...
  const contextTokens = positiveEnvironmentInteger(
    environment.DUGSYN_CONTEXT_TOKENS,
    "DUGSYN_CONTEXT_TOKENS",
  );
  const compressAt = positiveEnvironmentInteger(
    environment.DUGSYN_CONTEXT_COMPRESS_AT,
    "DUGSYN_CONTEXT_COMPRESS_AT",
  );
  const sessionDirectory = nonEmpty(environment.DUGSYN_SESSION_DIR);
  const model = nonEmpty(environment.DUGSYN_MODEL);
  return {
    ...
    ...(compressAt === undefined ? {} : { context: { compressAt } }),
  };
}
