...
async function workspace(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "dugsyn-extensions-"));
...
describe("Skills", () => {
  it("publishes metadata first, loads content on demand, and never loads sibling scripts", async () => {
    ...
    const projectDirectory = join(root, ".dugsyn", "skills", "review");
  ...
  it("ignores project Skills before trust and rejects a trusted symlink escape", async () => {
    ...
    await mkdir(join(root, ".dugsyn", "skills", "outside"), { recursive: true });
    await writeFile(join(outside, "SKILL.md"), "outside");
    await symlink(
      join(outside, "SKILL.md"),
      join(root, ".dugsyn", "skills", "outside", "SKILL.md"),
    );
