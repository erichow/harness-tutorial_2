...
async function fixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), "dugsyn-config-"));
  const configurationDirectory = join(root, ".dugsyn");
...
describe("layered configuration", () => {
  it("loads managed, user, project, and local in order after global trust", async () => {
    ...
    const secondTrustedRoot = await mkdtemp(join(tmpdir(), "dugsyn-config-trusted-"));
  ...
  it("rejects a trusted project config symlink that escapes the workspace", async () => {
    const { root, paths } = await fixture();
    const outside = join(await mkdtemp(join(tmpdir(), "dugsyn-config-outside-")), "config.json");
    await json(paths.user, { trustedWorkspaces: [root] });
  ...
  it("uses only product-prefixed runtime overrides and validates their values", async () => {
    ...
    const configuration = await loadConfiguration({
      ...
      environment: {
        PROVIDER: "ignored",
        MODEL: "ignored",
        DUGSYN_PROVIDER: "deepseek",
        DUGSYN_MODEL: "deepseek-env",
        DUGSYN_CONTEXT_TOKENS: "24000",
        DUGSYN_SESSION_DIR: "state",
      },
      paths,
    });
