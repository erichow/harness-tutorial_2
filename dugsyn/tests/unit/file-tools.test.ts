...
describe("workspace file tools", () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), "dugsyn-workspace-"));
    outside = await mkdtemp(join(tmpdir(), "dugsyn-outside-"));
  });
  ...
  it("rejects a stale version when the user edits while the agent is thinking", async () => {
    ...
    await expect(readFile(path, "utf8")).resolves.toBe("changed elsewhere\n");
    expect((await readdir(workspace)).some((name) => name.includes(".dugsyn-") && name.endsWith(".tmp"))).toBe(false);
  });
  ...
  it("paginates bounded output and invalidates a read cursor after a file change", async () => {
    ...
    // ~80KB output: 1500 lines of ~45 chars + line-number prefix triggers 64KB pagination
    await writeFile(path, Array.from({ length: 1500 }, (_, index) => `line-${String(index + 1).padStart(4, "0")}-${"x".repeat(36)}`).join("\n"));
    const executor = await tools();
