...
describe("chat arguments", () => {
  ...
  it("uses CLI, product-prefixed environment, then file defaults", () => {
    ...
    expect(parseChatArgs([], {
      ...
      DUGSYN_MODEL: "environment-model",
    ...
    ], { DUGSYN_MODEL: "environment-model" }, "/workspace", defaults)).toMatchObject({
      provider: "openai",
      model: "cli-model",
    });
  });
  ...
  it("allows resume and fork modes to inherit stored provider configuration", () => {
    expect(parseChatArgs(
      ...
      { DUGSYN_PROVIDER: "deepseek", DUGSYN_MODEL: "ignored" },
      "/workspace",
